...
class Graph(innerModel: MutableMap<String, Any>, val targets : MutableList<Target>) : Panel(innerModel) {

    companion object {
        fun parseGraph(model: Map<String, Any>) : Graph {
            val targets = (model["targets"] as List<Map<String, Any>>).map { Target(it.toMutableMap()) }.toMutableList()
